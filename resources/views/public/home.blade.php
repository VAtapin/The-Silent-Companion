...
<header class="absolute inset-x-0 top-0 z-20"><div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-6"><a href="{{ \App\Support\PublicUrl::home() }}" class="font-semibold">{{ $title }}</a><div class="flex items-center gap-3"><x-language-switcher public /><a href="{{ route('login') }}" class="rounded-full border border-white/20 px-4 py-2 text-sm hover:bg-white/10">{{ __('ui.team_login') }}</a></div></div></header>
<main>
