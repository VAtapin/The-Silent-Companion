...
<div x-data="{tab:'materials'}" class="mt-7"><div class="flex flex-wrap gap-2"><button @click="tab='materials'" :class="tab==='materials'?'bg-ink-900 text-white':'bg-white'" class="rounded-xl px-4 py-2 text-sm">Публикации</button><button @click="tab='site'" :class="tab==='site'?'bg-ink-900 text-white':'bg-white'" class="rounded-xl px-4 py-2 text-sm">Главная страница</button><button @click="tab='donations'" :class="tab==='donations'?'bg-ink-900 text-white':'bg-white'" class="rounded-xl px-4 py-2 text-sm">Пожертвования</button></div>
...
<section x-show="tab==='site'" class="mt-5 grid max-w-6xl gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
    <div class="card">
        ...
        <form method="POST" action="{{ route('publications.site') }}" enctype="multipart/form-data" class="mt-4 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-2xl border border-amber-400/50 bg-amber-50 p-4">
                <label>Загрузить новую афишу</label>
                <input type="file" name="poster_file" accept="image/jpeg,image/png,image/webp">
                <p class="mt-2 text-xs leading-5 text-slate-500">JPG, PNG или WEBP, до 25 МБ. Вертикальный формат 2:3 подходит лучше всего. Загруженный файл автоматически станет выбранной афишей.</p>
            </div>
            <div>
                <label>Или выбрать ранее загруженное изображение</label>
            ...
            <button class="btn-primary">Сохранить и разместить на главной</button>
        </form>
    </div>
