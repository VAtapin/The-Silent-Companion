...
<div x-data="{ nav: false }" class="min-h-screen lg:grid lg:grid-cols-[260px_1fr]">
    ...
    <aside :class="nav ? 'block' : 'hidden'" class="fixed inset-x-0 top-[52px] z-20 h-[calc(100vh-52px)] overflow-y-auto bg-ink-950 p-5 lg:sticky lg:top-0 lg:block lg:h-screen">
        ...
        @php($nav = [
            ...
            ['catalog.index', 'Персонажи', null, ['characters']], ['catalog.index', 'Места', null, ['locations']], ['catalog.index', 'Реквизит и одежда', null, ['props']], ['documents.index', 'Документы', 'documents.*'], ['ai.index', 'ИИ-помощник', 'ai.*'], ['publications.index', 'Публикации', 'publications.*'], ['team.index', 'Команда', 'team.*'], ['activity.index', 'История изменений', 'activity.*'],
        ])
